# NOTE: Based on parallel.py example, but using pymutex for locking
#       which is much faster but only works with libpthread on Unix/Linux.
#
# Two processes are incrementing a counter in parallel
#
# In this example we use the shared_lock='pymutex' parameter.
# This way of shared locking is safe accross independent
# processes but it is 50 % slower than using the built-in default
...
def run(name, target, x):
    ...
    d = UltraDict(name=name, shared_lock='pymutex')
...
if __name__ == '__main__':
    ...
    ultra = UltraDict(buffer_size=10_000, shared_lock='pymutex')
    ultra['counter'] = 0
